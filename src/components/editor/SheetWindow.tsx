...
export const SheetWindow = ({ sheetId }: Props) => {
  const { sheets, windows, focusWindow, closeWindow, minimizeWindow, maximizeWindow, updateWindowBounds } = useScratchpadStore()
  const sheet = sheets.find(s => s.id === sheetId)
  const win   = windows.find(w => w.sheetId === sheetId)

  // ── Estado local de geometria ─────────────────────────────────────────────
  ...
  const handleTitlebarPointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      lastX = ev.clientX
      lastY = ev.clientY
      setPos(prev => ({ x: prev.x + dx, y: Math.max(0, prev.y + dy) }))
    }
    const onUp = () => {
  ...
  const windowVariants = {
    ...
    visible:   { opacity: 1, scale: 1, y: 0, filter: 'blur(0px)', transition: { ...SPRING, filter: { duration: 0.2 } } },
    minimized: {
      opacity: 0, scale: 0.3, filter: 'blur(4px)',
      y: window.innerHeight - pos.y - 80,
      x: window.innerWidth / 2 - pos.x - size.width / 2,
      ...
      {!isMin && (
        ...
        >
          ...
          <div
            ...
            style={{
              background: isMax ? 'rgba(255,254,252,0.99)' : 'rgba(255,255,255,0.82)',
          ...
          >
            ...
            <div
              ...
              style={{
                height: 40,
                background: isMax ? 'rgba(250,249,246,0.99)' : 'rgba(255,255,255,0.55)',
            ...
            >
              ...
              >
                <motion.button
                  ...
                  whileHover={{ scale: 1.12 }} whileTap={{ scale: 0.82 }}
                  ...
                  <AnimatePresence>
                    {isHoveringTraffic && (
                      <motion.span initial={{ opacity:0, scale:0.6 }} animate={{ opacity:1, scale:1 }} exit={{ opacity:0, scale:0.6 }}
                        transition={{ duration: 0.08 }} className="absolute pointer-events-none"
              ...
              <div className="flex-1 flex items-center justify-center min-w-0 px-2">
                <span className="truncate" style={{
                  fontFamily: 'Lora, serif', color: 'rgba(0,0,0,0.42)',
                  fontSize: '0.78rem', letterSpacing: '0.015em',
              ...
              <div className="no-drag flex items-center gap-1 shrink-0">
                ...
                <span className="px-1.5 py-0.5 rounded-md" style={{
                  fontFamily: 'JetBrains Mono, monospace', color: 'rgba(0,0,0,0.22)',
                  background: 'rgba(79,110,247,0.07)', fontSize: '0.6rem', letterSpacing: '0.02em',
                }}>⌃↵</span>
              </div>
            </div>

            {/* Editor */}
            <div className="no-drag flex-1 overflow-hidden" onMouseDown={e => e.stopPropagation()}>
              <TipTapEditor key={sheet.id} sheetId={sheet.id} content={sheet.content} />
            </div>
          </div>
        </motion.div>
      )}
